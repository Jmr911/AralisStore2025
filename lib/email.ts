// lib/email.ts - VERSIÓN CORREGIDA CON SKU EN EMAILS
...
const EMAIL_CONFIG = {
  ...
  brandName: 'Aralis',
  supportEmail: 'jmr91_@hotmail.com', // 👈 Tu email personal (temporal)
...
export async function enviarEmailPedido(pedido: any) {
  try {
    // ✅ CORREGIDO: Agregar SKU en la generación del HTML de productos
    ...
    const { data, error } = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: pedido.email, // 👈 Solo al cliente (temporal)
      // 📝 NOTA: Cuando tengas dominio verificado, cambia a:
      // to: [pedido.email, 'AralisModa@hotmail.com'],
...
export async function enviarEmailCambioEstado(pedido: any, nuevoEstado: string) {
  try {
    const { data, error } = await resend.emails.send({
      from: EMAIL_CONFIG.from,
      to: pedido.userEmail || pedido.email, // 👈 Solo al cliente (temporal)
      // 📝 NOTA: Cuando tengas dominio verificado, cambia a:
      // to: [pedido.userEmail || pedido.email, 'AralisModa@hotmail.com'],
...
function generarHTMLCambioEstado(pedido: any, nuevoEstado: string): string {
  ...
  // ✅ CORREGIDO: Generar HTML de productos CON SKU (solo el código)
  const productosHTML = pedido.productos?.map((prod: any) => `
    <div style="padding: 15px 0; border-bottom: 1px solid #E8E6DD;">
      <div style="font-weight: 600; color: #2C1810; margin-bottom: 6px; font-size: 15px;">${prod.nombre}</div>
      ${prod.sku ? `<div style="font-size: 13px; color: #888888; margin-bottom: 3px; font-family: 'Courier New', monospace;">${prod.sku}</div>` : ''}
...
function generarTextoplanoPedido(pedido: any): string {
  // ✅ CORREGIDO: Incluir SKU en versión texto plano (solo el código)
